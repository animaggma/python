"""
Write a Python program that reads a large text file and counts the occurrences of each word in parallel using both multithreading and multiprocessing. Compare the performance of these two approaches.
...
Implement a multiprocessing version of the word counting function. Divide the file into chunks and assign each chunk processing to a separate process. Use the multiprocessing module for parallel processing.
Compare the execution time of the sequential word counting, multithreading version, and multiprocessing version for different file sizes. Use the time module to measure the execution time.
Display the results, including the execution time for each approach and the speedup achieved by multithreading and multiprocessing compared to the sequential version.
